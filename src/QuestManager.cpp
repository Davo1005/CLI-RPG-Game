...
void QuestManager::assign_quest(Hero* hero,Quest* quest){
        printslow(hero->get_name() + "Assign" + quest->get_description());
}
...
void QuestManager::displayActiveQuests()const{
        for(int i=0;i<activeQuests.size();i++){
                printslow(activeQuests[i]->get_description());
        }
}
Quest* QuestManager::get_Quest(){
        if(activeQuests.empty()){
                printslow("There is no quest for you");
                return nullptr;
        }
        else{
...
void QuestManager::displayCompletedQuests()const{
        for(int i=0;i<completedQuests.size();i++){
                ...
                {
                        printslow(completedQuests[i]->get_description());
                }
        }
}
QuestManager::~QuestManager(){
